...
export default function UserTypes() {
  const shipperAvatars = [Shipper3, Shipper2, Shipper5, Shipper4, Shipper1];
  ...
  const singularUserTypes: UserType[] = [
    {
      ...
      description:
        "Shippers/Agents are individuals who directly or indirectly have  cargo in need of Transport. We’ve created revolutionary ways to manage your cargo delivery.",
      actionLink: "",
    ...
    },
    {
      ...
      description:
        "These are professional drivers or Truck owners who help move cargo for those that need cargo transport. With truckdispatch, Interaction with agents/shippers and companies have never been easier.",
      actionLink: "",
  ...
  const organizationalUserTypes: UserType[] = [
    {
      label: "Companies",
      description:
        "For Companies with cargo transport needs, we have a large pool of vetted and trusted transporters/individuals and transport companies ready to handle your cargo. You can transport huge volumes and  large cargos easily and safe.",
      actionLink: "",
    },
    {
      label: "Transport Companies",
      description:
        "For Transport companies of any size, we have a complete toolbox of practical solutions to manage your every day hassles",
      actionLink: "",
    },
  ];
      ...
      <div className={styles.user_types_banner}>
        ...
        <div className={styles.user_types_banner__content}>
          <h2>
            ...
            <span className={styles.purple_text}>Solve</span> any cargo
            transport problems.
          </h2>
          <p>
            Whether you’re an individual or a company, we have solutions
            available for you. See the category that best fits you.
          </p>
        </div>
      </div>

      <div className={styles.user_types_grid}>
        {singularUserTypes.map((userType) => (
          <div key={userType.label} className={styles.gray_bg}>
            <UserTypeDetails userType={userType} />
          </div>
        ))}
      </div>
      <div className={styles.user_type_without_avatar_grid}>
        {organizationalUserTypes.map((userType, index) => (
          <div key={index} className={styles.user_type_item}>
            <UserTypeDetails userType={userType} />
          </div>
        ))}
      </div>
